...
const inlineKeyboard_WeekMatch = (
  ctx: any,
  data: any,
  edit: boolean,
  totalPage: number,
  ...
  if (edit) {
    buttons = matchButtons.concat([
      [
        Markup.button.callback('◀️', 'previous_WEEK_MATCH'),
        Markup.button.callback('⏪', 'top_WEEK_MATCH'),
        Markup.button.callback('⏩', 'end_WEEK_MATCH'),
        Markup.button.callback('▶️', 'next_WEEK_MATCH'),
      ],
      [
        Markup.button.webApp(
          ctx.i18n.t('WEEK_MATCH.allMatch'),
...
export default inlineKeyboard_WeekMatch;
